  ...
  const mentors = [
    {
      ...
      socials: {
        ...
        website: "https://brandonlastomo.vercel.app/",
        github: "https://github.com/BrandonLastomo/",
        dribbble: "https://dribbble.com/BrandonHL"
      }
    },
    {
      name: "Aldi Wijaya",
      role: "Web Development",
      image: "/studyclub/images/mentor%20webdev.png",
      decorations: [
  ...
  return (
    <section id="mentor" className="relative w-full bg-[#01031A] py-24 flex flex-col items-center overflow-hidden">
      ...
      <div className="max-w-7xl w-full mx-auto px-6 flex flex-col items-center z-10" ref={popupRef}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-12 w-full max-w-5xl mx-auto">
                ...
                <div className="relative">
                  ...
                  >
                    <svg 
                      ...
                      strokeWidth={2} 
                      stroke="currentColor" 
                      className={`w-5 h-5 transition-transform duration-300 ${activePopup === index ? 'rotate-90' : ''}`}
                    >
                      {activePopup === index ? (
                  ...
                  <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 transition-all duration-300 ${activePopup === index ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
                    <a 
                      ...
                      className={`absolute w-12 h-12 bg-[#0A66C2] rounded-full flex items-center justify-center text-white hover:scale-110 transition-all duration-500 shadow-lg ${activePopup === index ? '-translate-y-[110px] translate-x-0 md:translate-x-[45px] md:-translate-y-[45px] scale-100' : 'translate-x-0 translate-y-0 scale-50 opacity-0'}`}
                    ...
                    <a 
                      ...
                      rel="noopener noreferrer" 
                      onClick={(e) => e.stopPropagation()}
                      className={`absolute w-12 h-12 bg-gradient-to-tr from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] rounded-full flex items-center justify-center text-white hover:scale-110 transition-all duration-500 shadow-lg ${activePopup === index ? '-translate-y-[95px] -translate-x-[55px] md:translate-x-[60px] md:translate-y-[10px] scale-100' : 'translate-x-0 translate-y-0 scale-50 opacity-0'}`}
                    ...
                    {mentor.socials.website && (
                      <a 
                        ...
                        rel="noopener noreferrer" 
                        onClick={(e) => e.stopPropagation()}
                        className={`absolute w-12 h-12 bg-[#2563EB] rounded-full flex items-center justify-center text-white hover:scale-110 transition-all duration-500 shadow-lg ${activePopup === index ? '-translate-y-[55px] -translate-x-[95px] md:translate-x-[30px] md:translate-y-[65px] scale-100' : 'translate-x-0 translate-y-0 scale-50 opacity-0'}`}
                    ...
                    <a 
                      ...
                      rel="noopener noreferrer"
                      onClick={(e) => e.stopPropagation()}
                      className={`absolute w-12 h-12 bg-[#181717] rounded-full flex items-center justify-center text-white hover:scale-110 transition-all duration-500 shadow-lg ${
                        activePopup === index 
                          ? (mentor.socials.website 
                              ? 'translate-y-0 -translate-x-[110px] md:-translate-x-[25px] md:translate-y-[65px] scale-100'
                              : '-translate-y-[55px] -translate-x-[95px] md:translate-x-[30px] md:translate-y-[65px] scale-100')
                          : 'translate-x-0 translate-y-0 scale-50 opacity-0'
                      }`}
                      style={{ transitionDelay: activePopup === index ? (mentor.socials.website ? '200ms' : '150ms') : '0ms' }}
                    >
                    ...
                    {mentor.socials.dribbble && (
                      <a 
                        href={mentor.socials.dribbble} 
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={(e) => e.stopPropagation()}
                        className={`absolute w-12 h-12 bg-white rounded-full flex items-center justify-center hover:scale-110 transition-all duration-500 shadow-lg ${activePopup === index ? 'translate-y-[55px] -translate-x-[95px] md:-translate-x-[75px] md:translate-y-[30px] scale-100' : 'translate-x-0 translate-y-0 scale-50 opacity-0'}`}
                        style={{ transitionDelay: activePopup === index ? '250ms' : '0ms' }}
                      >
                        <img 
